  ...
  return (
    <Box
      sx={{
        ...
        overflow: { md: "auto" },
        bgcolor: "#F5F5F5",
        borderLeft: { md: "1px solid #FFCDD2" },
    ...
    >
      ...
      {user && (
        ...
        >
          ...
          <Box>
            <Typography variant="subtitle2" fontWeight="bold" sx={{ color: "#212121" }}>
              {user.username || "User"}
            </Typography>
            <Typography 
              variant="body2" 
              sx={{ color: "#606060", display: { xs: 'none', sm: 'block' } }}
            >
              {user.email}
            </Typography>
          </Box>
          ...
          .map((u) => (
            ...
            >
              <Avatar
                ...
                src={u.profileImage ? `http://localhost:9090${u.profileImage}` : undefined}
                alt={u.username}
              >
                {!u.profileImage && u.username?.charAt(0).toUpperCase()}
              </Avatar>
              <Box sx={{ flex: 1 }}>
                <Typography variant="subtitle2" fontWeight="600" sx={{ color: "#212121" }}>
                  {u.username}
              ...
              {followStatus[u.id] === "ACCEPTED" ? (
                ...
                <Button
                  ...
                  sx={{
                    color: followStatus[u.id] === "PENDING" ? "#606060" : '#FF0000',
                    textTransform: 'none',
                    fontWeight: '600',
                    fontSize: '0.75rem',
                    p: 0.5,
                    "&:hover": { color: followStatus[u.id] === "PENDING" ? "#606060" : "#D32F2F" }
                  }}
                >
      ...
      {/* Footer Links */}
      <Box sx={{ mt: 'auto', pt: 2 }}>
        <Typography variant="caption" sx={{ color: "#606060", mb: 1 }}>
          © 2025 Skillplus
        </Typography>
